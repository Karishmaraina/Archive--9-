...
const Sidebar = ({ userId, setCurrentConvo, handleAddGroup }) => {
  ...
  // Fetch conversations dynamically based on filter & search query
  ...
  //     try {
  //       const data = await fetchFilteredChats(userId, filter, searchQuery);
  ...
  return (
    <div className="w-[45%] max-w-[800px] h-full bg-white py-2 px-3 relative flex flex-col">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex items-center space-x-2">
          <div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center text-white text-lg">
            {localStorage.getItem("userName")?.charAt(0).toUpperCase() || <FaUserCircle size={30} />}
          </div>
            ...
            {["all", "unread", "groups"].map((type) => (
              <button
                ...
                className={`px-3 py-1 rounded-full transition ${
                  filter === type ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
                }`}
      ...
      {mode === "chats" && <Conversations conversations={conversations} selectConvo={setCurrentConvo} />}
      ...
      {mode === "group" && <Group goBack={() => setMode("chats")} handleGroupCreated={handleAddGroup} />}
